...
전종목 한 파일이라 파일 캐시 + 프로세스 캐시한다. companyfacts 는 종목별 큰 JSON(수 MB)이므로 그대로 반환.
"""
...
# 파일 캐시: 프로세스 재시작 후 cold start 방지. TTL 24h (SEC은 주 1회 갱신).
_TICKER_CACHE_TTL = 86400  # seconds
_TICKER_CACHE_PATH = Path(__file__).parent / "ticker_map.json"
...
def _load_ticker_map_from_file() -> tuple[dict[str, dict], bool] | None:
    """파일 캐시 로드. (map, is_fresh) 또는 None."""
    try:
        if not _TICKER_CACHE_PATH.exists():
            return None
        mtime = _TICKER_CACHE_PATH.stat().st_mtime
        if time.time() - mtime > _TICKER_CACHE_TTL:
            return None
        with open(_TICKER_CACHE_PATH, encoding="utf-8") as f:
            return json.load(f), True
    except Exception:
        return None


def _save_ticker_map_to_file(data: dict[str, dict]) -> None:
    try:
        with open(_TICKER_CACHE_PATH, "w", encoding="utf-8") as f:
            json.dump(data, f)
    except Exception as e:
        logger.warning("ticker map file save failed: %s", e)


def ticker_map(settings: Settings, session: requests.Session | None = None) -> dict[str, dict]:
    """전종목 ticker→{cik:int, name:str} 매핑. 파일 캐시(24h TTL) → 프로세스 캐시 → SEC fetch. 실패 시 빈 dict."""
    global _ticker_map
    if _ticker_map is not None:
        return _ticker_map

    # 1. 파일 캐시에서 먼저 시도 (cold start 방지).
    cached = _load_ticker_map_from_file()
    if cached is not None:
        _ticker_map = cached[0]
        return _ticker_map
    ...
    _ticker_map = out
    _save_ticker_map_to_file(out)
    return out


def resolve_cik(
    settings: Settings, ticker: str, session: requests.Session | None = None
) -> int | None:
