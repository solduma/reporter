...
# H-Model 감쇠기간(2H, 년) — 초과수익(해자)이 소멸하는 지평. 종목별 산정(fade_years 참조).
# 산정 실패(입력 결측) 시 이 기본값을 쓴다.
DEFAULT_FADE_YEARS = 8
FADE_YEARS_MIN = 3   # 해자 약함·범용재
FADE_YEARS_MAX = 20  # 강한 해자(네트워크·브랜드·규제)
NEAR_TERM_GROWTH_CAP = 0.30  # 단기 성장 g_S 상한(장기 지속 불가한 과도 추정 방어).

# 해자(LLM 정성 판정) → 감쇠기간 배수. 정량 기준선을 조정한다.
MOAT_MULTIPLIER = {"강": 1.4, "중": 1.0, "약": 0.6}


def fade_years(roe: float | None, wacc_val: float | None, moat: str | None) -> tuple[float, list[str]]:
    """H-Model 고성장 감쇠기간(년) = 정량 기준선 × 정성 해자배수. 앙상블.

    정량: 초과수익 스프레드(ROE−WACC)가 클수록 해자가 길다 → 기준선 3~15년 선형(스프레드 0%→3년,
    10%p↑→15년). 정성: LLM 해자 판정(강·중·약)으로 ×0.6~1.4. 최종 clamp(3~20년). 과정 스텝 반환.
    입력 결측 시 DEFAULT_FADE_YEARS.
    """
    if roe is None or wacc_val is None:
        return DEFAULT_FADE_YEARS, [f"감쇠기간 {DEFAULT_FADE_YEARS}년(기본 — ROE·WACC 결측)"]
    # ROE 는 % 단위(예 12.0)로 들어올 수 있어 소수로 정규화(>1 이면 /100).
    roe_frac = roe / 100 if abs(roe) > 1 else roe
    spread = roe_frac - wacc_val  # 초과수익률(자본비용 대비)
    base = 3.0 + max(0.0, min(spread, 0.10)) / 0.10 * 12.0  # 0%→3년, 10%p→15년
    mult = MOAT_MULTIPLIER.get(moat or "", 1.0)
    years = max(FADE_YEARS_MIN, min(FADE_YEARS_MAX, base * mult))
    steps = [
        f"초과수익 스프레드 ROE {roe_frac:.1%} − WACC {wacc_val:.1%} = {spread:+.1%} → 기준 감쇠 {base:.1f}년",
        f"해자 '{moat or '중'}' 배수 ×{mult:g} → 감쇠기간 {years:.1f}년(H={years / 2:.1f})",
    ]
