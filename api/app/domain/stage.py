...
def segments(
    ...
    for i in range(ma_period - 1, len(closes)):
        ...
        if prev_stage in (1, 3) and r.price_pos == "near" and r.ma_dir == "flat":
            st = prev_stage
        if st is not None:
            raw.append((st, dates[i], r.price_pos or "near", r.ma_dir or "flat"))
            prev_stage = st
    if not raw:
        return []
    ...
    out: list[dict] = []
    committed_stage, committed_from, committed_pos = raw[0][0], raw[0][1], raw[0][2]
    cand_stage, cand_from = raw[0][0], raw[0][1]
    cand_len = 1
    last_date = raw[0][1]

    for st, d, pos, madir in raw[1:]:
        last_date = d
        if st == cand_stage:
            cand_len += 1
        else:
        ...
        if cand_stage != committed_stage:
            ...
            # MA 하향 이탈 하락(4·below)으로의 진입, 또는 상승 MA 를 타는 상승(2·madir=rising)으로의 진입.
            # 바닥에서의 급락이 min_run 문턱에 걸려 바닥밴드로 흡수되던 문제 방지(예: 코로나 급락).
            # 상방 돌파(Stg2 진입)는 평탄 MA 위 거짓 돌파가 잦아 원칙적으로 확인을 요구하되, MA 가 이미
            # '상승 중'인 돌파는 고확신 추세 진입이라 빠른 확정에 포함한다(상승 MA 위 실제 마크업이 min_run
            # 문턱을 한 봉 차로 놓쳐 통째 바닥으로 남던 문제 방지 — 인피니트헬스케어 2026 마크업).
            strong = (
                ...
                or (cand_stage == 2 and madir == "rising")
            )
...
def _direction_correct(
    seg: dict, close_at: dict[str, float], raw: list[tuple[int, str, str, str]]
) -> list[dict]:
    ...
    if stg == 3 and ret > STG3_ADVANCE:
        ...
        prices = [close_at[item[1]] for item in items if item[1] in close_at]
...
def _tail_correct(seg: dict, raw: list[tuple[int, str, str, str]]) -> list[dict]:
...
def _prev_day(raw: list[tuple[int, str, str, str]], date_str: str) -> str:
