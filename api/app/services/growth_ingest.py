...
def _ingest_one(db: Session, code: str, snap_date: date, session: requests.Session) -> None:
    ...
    if metric:
        stmt = insert(GrowthMetricRow).values(
            ...
            net_status=metric.net_status,
            net_margin_delta=metric.net_margin_delta,
            ebitda_status=metric.ebitda_status,
            ebitda_margin_delta=metric.ebitda_margin_delta,
        )
        stmt = stmt.on_conflict_do_update(
            ...
            set_={
                ...
                "net_status": stmt.excluded.net_status,
                "net_margin_delta": stmt.excluded.net_margin_delta,
                "ebitda_status": stmt.excluded.ebitda_status,
                "ebitda_margin_delta": stmt.excluded.ebitda_margin_delta,
