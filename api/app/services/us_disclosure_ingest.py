...
수집 시엔 LLM 을 쓰지 않고 item 코드→한글 요약(sec.describe_8k_items)만 title 에 넣어 비용 0.
...
def sync_8k(db: Session, ticker: str, settings: Settings, session: requests.Session) -> int:
    ...
    cik = sec.resolve_cik(settings, ticker, session)
    if cik is None:
        return 0
    filings = sec.fetch_recent_filings(settings, cik, forms=("8-K",), limit=_PER_FILING, session=session)
    saved = 0
    for f in filings:
        values = {
            "ticker": ticker,
            ...
            "title": sec.describe_8k_items(f.items),
        }
        stmt = (
            insert(UsDisclosure)
