...
# 이벤트·뉴스 보존일. 스크리너 윈도우(14일)보다 넉넉히 두고 그 이전은 삭제(무한 증가 방지).
_RETENTION_DAYS = 30
...
def run_news_events(db: Session, settings: Settings | None = None) -> dict:
    ...
    # 분류 완료된 link 만 건너뛴다(멱등). 분류 실패로 classified=False 인 뉴스는 재시도한다
    # (일시적 LLM 오류로 이벤트에서 영구 누락되지 않도록).
    done = set(
        db.scalars(
            select(NewsArticle.link).where(
                NewsArticle.link.in_([it.link for it in items]),
                NewsArticle.classified.is_(True),
            )
        ).all()
    )
...
def _purge_old(db: Session, now: datetime) -> int:
