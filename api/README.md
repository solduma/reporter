...
uv sync
uv run uvicorn app.main:app --port 38010 --reload
```
...
```bash
make tui           # 프로젝트 루트에서 (권장)
# 또는
cd api && uv run reporter-tui
```

- **상태 패널**: 테이블 행수(reports/universe/growth/…)·최신 스냅샷 날짜 (`r` 새로고침).
- **서버 제어**: API(:38010)·WEB(:43000) 은 **launchd 서비스**(`com.reporter.server.*`)로 상시
...
```bash
cd web
cp .env.local.example .env.local   # NEXT_PUBLIC_API_BASE=http://127.0.0.1:38010
pnpm install
