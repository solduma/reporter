...
|---|---|---|---|
| 1 Overview | per/pbr/시총/주주구성/사업개요 | universe·financials·DART **최근 정기보고서** | 낮음(대부분 바인딩) |
| 2 Red Flags | 매출채권·재고·OCF 괴리, 무형자산 비중, 현금 시계열 | financials 시계열·DART 재무제표 주석 | 중(주석 조회 반복) |
| 3 Business | 밸류체인·벤더/납품처·경쟁사·아이템 비중 변화 | DART 사업보고서 → 리포트(개별+산업) → 웹(네이버 블로그) | **높음** |
...
### 개선사항 (사용자 지시 반영)
- **1단계 Overview 정기보고서 선택**: 사업보고서만이 아니라 반기·분기보고서도 대상. `report_nm`
  기준일이 아니라 **rcept_dt(접수일) 최신** 정기공시를 골라 가장 신선한 문서를 쓴다
  (최근 2회계연도의 annual/half/quarter 를 조회해 접수번호 최대값 = 최신).
- **3단계 Business 탐색 순서**: `사업보고서 → 리포트 → 웹` 순으로 시도.
  - 리포트는 **개별 종목 리포트가 없어도** 이 종목이 속한 산업 리포트(`category=industry`,
    같은 `industry_name`)에서 언급을 찾고, 개별 리포트 본문(rationale)에서 이 종목명 언급도 검색.
  - 그래도 부족하면 **웹(네이버 블로그 기업 리서치 글 등)** 으로 보강.

## 5. 도구 카탈로그 (tools.py — 기존 자산 래핑)

1차 도구(대부분 기존 어댑터/서비스 재사용 + 웹 검색 신규 어댑터):
- `get_recent_periodic_report(code)` — 최신 정기보고서(사업/반기/분기 중 접수일 최신) rcept_no·본문
...
- `get_reports(code)` — 개별 종목 리포트 + 이 종목이 속한 산업 리포트 + 본문 내 종목명 언급
- `get_peers(code)` — 동종 밸류에이션 (company_service.peer_valuations)
- `get_price_context(code)` — 현재가·모멘텀·52주 (company_service)
- **`web_search(query)`** — 웹 검색(1차 필수). 네이버 블로그 기업 리서치 글 우선 + 일반 웹.
- **`fetch_web_page(url)`** — 검색 결과 URL 본문 추출(네이버 블로그 iframe 해제 포함).

### 웹 검색 어댑터 (신규, adapters/websearch)
한국 종목 리서치는 네이버 블로그에 개인 애널리스트·투자자의 심층 글이 많아 핵심 소스. 구현:
- 검색: 네이버 검색 API(블로그/웹문서, client_id/secret) 우선. 키 없으면 대체 검색 엔진 폴백.
- 블로그 본문: blog.naver.com 은 iframe(`PostView.naver?blogId=&logNo=`) 구조 → iframe 실제 URL로
  재요청 후 본문 셀렉터로 텍스트 추출(bs4, 기존 스크래핑 패턴 재사용).
- 구체 엔드포인트·인증·셀렉터는 리서치 결과로 확정(별도 조사 중). 어댑터 뒤로 감춰 서비스는 도구만 호출.
