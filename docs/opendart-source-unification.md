...
3. ✅ **DS003 재무지표 → ROE** (#431 완료): fnlttSinglIndx 수익성지표(M210000)의 ROE 를 백필로 적재(2023~). 2023 3Q 이전은 네이버 폴백. per/pbr/psr·ev_ebitda 는 기존 소유자 유지(불간섭).
4. ❌ **DS003 전체재무제표 → D&A** (대체 보류 — 원문 파싱 유지): A/B 검증 결과 구조화 API(fnlttSinglAcntAll CFS)에 대형사 D&A add-back 이 **부재**(삼성·SK하이닉스·현대차 2024 모두 CF 계정에 감가/무형상각 0개, account_id 표준태그도 없음). 구조화 API로 대체하면 대형사 EBITDA 과소계산 회귀 → `report_parser.py` 원문 XML 파싱 유지가 정답. 선행조건 부정으로 판정, 코드 변경 없음.
