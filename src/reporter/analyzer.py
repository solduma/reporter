...
_INTRADAY_SYSTEM = (
    ...
    "직전 시황은 '무엇이 새로워졌는지'를 가려내는 참고용일 뿐이다 — 직전 내용을 되풀이하거나 누적하지 "
    "말고, 지금 새로 들어온 수치·뉴스·흐름을 중심으로 쓴다. 분량 비중은 새로운 시황 정보 9 : "
    "직전 대비 변화 요약 1 이다."
)
...
위 실시간 데이터를 근거로 '지금 장 상황'을 아래 형식으로 작성해라. 직전 시황은 '새로 달라진 것'을
판별하는 참고용으로만 쓴다 — 직전 시황 내용을 그대로 옮기거나 누적하지 말고, 지금 새로 들어온
수치·뉴스·흐름을 중심으로 작성한다. 분량 비중은 새로운 시황 정보 9 : 직전 대비 변화 요약 1.
형식과 이모지를 그대로 지키고, 각 섹션 사이는 빈 줄로 띄운다. 지수·수치·종목명 등 핵심어만 굵게(**...**) 강조(남발 금지).

📊 지금 장 상황 (3줄)
→ 코스피/코스닥 현재 등락과 그 배경 (제시된 지수·환율 수치를 인용, 직전 대비 새로 달라진 점이 있으면 한 줄만)

🔥 움직이는 테마·뉴스
→ 최신 장중 뉴스가 가리키는 주도 흐름 2~3개 (직전 시황에 없던 새 재료 우선)
